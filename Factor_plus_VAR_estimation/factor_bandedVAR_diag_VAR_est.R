...
compute_errors = function(data, table, distr){
  ...
  for(truncs in c(T,F)){
    ...
    for(i in names(data)){
      pca_lasso = mapply(trunc_fit_diff_comm, data[[i]], common_comps[[i]], A_coeff = A_coeff[i], trunc = truncs, SIMPLIFY = F)
...
#### save ####
# saveRDS(lasso_fac_ke_table, file = "factor_plus_VAR/fac_var_lasso_cv_with_auto_banded_gauss.rds")
