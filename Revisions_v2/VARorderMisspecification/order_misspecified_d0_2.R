...
sparsevar_trunc_fit_diff = function(data, n_tau = 60, lag = d, trim = 1, trim_d=1, trim_all=F, A_coeff, max = T, m_norm, cv_lag=T){
  ...
  fit_diff = cbind(fit_diff$A[[1]], fit_diff$A[[2]], fit_diff$A[[3]]) - A_coeff
  return(fit_diff)
}
...
compute_errors = function(data, table, distr){
  ...
  for(i in names(data)){
    print(i)
    fit_diff = parallel::mclapply(data[[i]], sparsevar_trunc_fit_diff, A_coeff = A_coeff_ex[[i]], mc.cores = 4)
  ...
  # not-truncated
  for(i in names(data)){
    fit_diff = parallel::mclapply(data[[i]], function(x){
      fit = fitVAR(x, p = d, parallel = T, ncores = 4)
      cbind(fit$A[[1]], fit$A[[2]], fit$A[[3]])  - A_coeff_ex[[i]]
    },
    mc.cores =4)
...
saveRDS(lasso_table, file = "VARorder_misspecific_t4_d0_2_d_3.rds")
rm(lasso_table)
gc()
