...
compute_errors = function(data, table, distr){
  ...
  for(i in names(data)){
    pca_lasso = mapply(trunc_fit_diff_comm, data[[i]], common_comps[[i]], A_coeff = attr(data, "A_var")[[i]], SIMPLIFY = F)
...
# t_2.1
set.seed(200)
data = factor_mod_data_VAR_idio(dist = "t", innov_df = 2.1, n_p = n_p, fac_var_ret = T, norm_loading = T, indo_A = renyi_coeff)
system.time({
...
# t_3
# set.seed(200)
# data = factor_mod_data_VAR_idio(dist = "t", innov_df = 3, n_p = n_p, fac_var_ret = T, norm_loading = T, indo_A = renyi_coeff)
# lasso_fac_ke_table = compute_errors(
#   data = data,
...
# t_4
# set.seed(200)
# data = factor_mod_data_VAR_idio(dist = "t", innov_df = 4, n_p = n_p, fac_var_ret = T, norm_loading = T, indo_A = renyi_coeff)
# lasso_fac_ke_table = compute_errors(
#   data = data,
...
# gaussian
# set.seed(200)
# data = factor_mod_data_VAR_idio(dist = "gauss", n_p = n_p, fac_var_ret = T, norm_loading = T, indo_A = renyi_coeff)
# lasso_fac_ke_table = compute_errors(
#   data = data,
...
# lognormal
set.seed(200)
data = factor_mod_data_VAR_idio(dist = "log", n_p = n_p, fac_var_ret = T, norm_loading = T, indo_A = renyi_coeff)
lasso_fac_ke_table = compute_errors(
  data = data,
...
saveRDS(lasso_fac_ke_table, file = "Renyi_cellwise_standardised_fac_var_est_lognormal.rds")
