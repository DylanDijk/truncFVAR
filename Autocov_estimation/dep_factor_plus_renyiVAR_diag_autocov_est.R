...
# dependent samples, generated from Factor plus VAR(1) model with renyi random graph coefficient matrix, innovation errors with diagonal covariance structure. 
...
compute_errors = function(data, distr, table, m_norm, norm_loading = T){
  ...
  names(cov_var_i) = names(A_coeff_banded(n_p))  
  
  for(i in 1:nrow(n_p)){
    cov_var_i[[i]] = vector(mode = "list", length = nsim)
    for(k in 1:nsim){
      cov_var_i[[i]][[k]] = as.matrix(round(cov_of_var(A = attr(data, "A_var")[[i]][[k]]), 3))
    }  
  } 
  ...
  for(i in 1:nrow(n_p)){
    ...
    for(k in 1:nsim){
      ...
      cov_var_comb[[i]][[k]] = (lambda %*% cov_var_fac[[i]][[k]]) %*% t(lambda) + cov_var_i[[i]][[k]]
    }
  }
  ...
  for(i in 1:nrow(n_p)){
    ...
    for(k in 1:nsim){
      ...
      autocov_var_comb[[i]][[k]] = (lambda %*% autocov_var_fac[[i]][[k]]) %*% t(lambda)  +  
        (attr(data, "A_var")[[i]][[k]]) %*% cov_var_i[[i]][[k]]
    }
  }
...
set.seed(200)
ke_table = compute_errors(data = factor_mod_data_VAR_idio(dist = "gauss", n_p = n_p, indo_A = renyi_coeff, norm_loading = T),
...
for(i in c(2.1, 3, 4)){
  set.seed(200)
  ke_table = compute_errors(data = factor_mod_data_VAR_idio(dist = "t", innov_df = i, n_p = n_p, indo_A = renyi_coeff, norm_loading = T),
                            table = ke_table, distr = paste("t",sep = "_", i))
}

# log normal
set.seed(200)
ke_table = compute_errors(data = factor_mod_data_VAR_idio(dist = "log", n_p = n_p, indo_A = renyi_coeff, norm_loading = T),
